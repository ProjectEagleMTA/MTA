                                  ...
                                  const SLineOfSightFlags flags, SLineOfSightBuildingResult* pBuildingResult, SProcessLineOfSightMaterialInfoResult* outMatInfo)
{
    ...
    if (outMatInfo && targetEntity && targetEntity->m_pRwObject)
    {
        ...
        {
            float       minHitDistSq;
            CVector     originOS, endOS, dirOS;            // Line origin and end [in object space]
            CMatrix     entMat, entInvMat;
            RpGeometry* hitGeo{};
            RpTriangle* hitTri{};
        ...
        const auto ProcessOneAtomic = [](RpAtomic* a, void* data)
        {
            ...
            // TODO: Do bounding sphere check against line to early out all this expensive stuff
            ...
            for (auto i = geo->triangles_size; i-- > 0;)
            {
                ...
                CVector hitBary;
                CVector hitPos;
                if (!c->originOS.IntersectsSegmentTriangle(c->dirOS, verts[tri->verts[0]], verts[tri->verts[1]], verts[tri->verts[2]], &hitPos, &hitBary))
                {
                    continue;            // No intersection at all
                }

                // Intersection, check if it's closer than the previous one
                const auto hitDistSq = (hitPos - c->originOS).LengthSquared();
        ...
        if (outMatInfo->valid = c.hitGeo != nullptr)
        {
            ...
            outMatInfo->hitPos = c.entMat.TransformVector(c.hitPos);
        }
    }
