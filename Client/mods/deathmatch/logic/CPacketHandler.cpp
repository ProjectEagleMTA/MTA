...
/*
    // Проверка на хук в функции t_LuaLoadBuffer
    ...
    // Попробуем получить дескриптор модуля lua5.1c.dll
    HMODULE luaModule = GetModuleHandleA("lua5.1c.dll");
    if (!luaModule) {
        // Если модуль не найден, завершить процесс
        TerminateProcess(GetCurrentProcess(), 1);
    }
    
    // Попробуем получить адрес функции luaL_loadbuffer
    FARPROC procAddress = GetProcAddress(luaModule, "luaL_loadbuffer");
    if (!procAddress) {
        // Если функция не найдена, завершить процесс
        TerminateProcess(GetCurrentProcess(), 1);
    }
    
    // Используем reinterpret_cast для приведения типа
    callLuaLoadBuffer = reinterpret_cast<t_LuaLoadBuffer>(reinterpret_cast<void*>(procAddress));
    
    // Проверка на JMP инструкцию (0xE9) в начале функции
    if (*(BYTE*)callLuaLoadBuffer == 0xE9) {
        // Если обнаружен хук, завершить процесс
        TerminateProcess(GetCurrentProcess(), 1);
    }
*/
    // Can we handle it?
