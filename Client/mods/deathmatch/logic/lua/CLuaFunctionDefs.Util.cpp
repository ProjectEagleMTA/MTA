...
Custom Function NEXTRP

*********************************************/

// int CLuaFunctionDefs::takeScreenShot(lua_State* luaVM)  // Функция с NEXTRP( скриншот )
// {
...
int CLuaFunctionDefs::GetLauncherData(lua_State* luaVM)            // Функция с NEXTRP( лаунчер )
{
    ...
    // Пишем функцию под некста, hwid и ssid
    ...
    lua_pushnumber(luaVM, 105); // major для обязательных обнов
    lua_settable(luaVM, -3);

    lua_pushstring(luaVM, "minor_version");
    lua_pushnumber(luaVM, 1); // minor версия, для необязательных обнов
    lua_settable(luaVM, -3);

    // Для бета версии
    lua_pushstring(luaVM, "is_test_version");
    lua_pushstring(luaVM, false); // false для релиз клиентов, "true" для тестовых
    lua_settable(luaVM, -3);
