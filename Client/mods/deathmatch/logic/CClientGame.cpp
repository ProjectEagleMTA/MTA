...
void CClientGame::DoPulsePostFrame()
{
    ...
    if (m_pManager->IsGameLoaded())
    {
        ...
        if (m_pNetAPI->IsNetworkTrouble())
        {
            ...
            g_pCore->GetGraphics()->DrawString(iPosX, iPosY, iPosX, iPosY, COLOR_ARGB(255, 255, 0, 0), "*** Поганий зв'язок з інтернетом ***", 2.0f, 2.0f,
                                               DT_NOCLIP | DT_CENTER);
        }
