...
public:
    ...
    void UpdateProgressBarVisigle(bool state);
    void SetProgressBarDownloadProgress(uint64_t downloadedSizeTotal);
...
private:
    ...
    //CGUIStaticImage*                                 t_progress_bar_bg;
    //  ”даление фона
    CGUIStaticImage*                                 t_progress_bar_bg = reinterpret_cast<CGUIStaticImage*>(m_GUI->CreateStaticImage());
    CGUIStaticImage*                                 pProgressBG = reinterpret_cast<CGUIStaticImage*>(m_GUI->CreateStaticImage());
