...
void CLuaEngineDefs::LoadFunctions()
{
    constexpr static const std::pair<const char*, lua_CFunction> functions[]{
        ...
        {"setLoadingScreenVisible", ArgumentParser<setLoadingScreenVisible>},
        {"IsLoadingScreenVisible", ArgumentParser<IsLoadingScreenVisible>},
        {"takeScreenShot", ArgumentParser<takeScreenShot>},
        {"exitGame", ArgumentParser<exitGame>},
        {"settingsGame", ArgumentParser<settingsGame>},
        {"getLaunchData", GetLaunchData},
        {"removeModelFromMemory", RemoveModelFromMemory},
        {"setClientDrawDistance", SetClientDrawDistance},
        {"getClientMemoryInfo", GetClientMemoryInfo},
        {"getClientDrawDistance", GetClientDrawDistance}
...
bool CLuaEngineDefs::setLoadingScreenVisible(bool bVisible)
{
    g_pCore->RemoveMessageBox(true);
    g_pCore->SetVisibleMainMenu(bVisible);
    return true;
}

void CLuaEngineDefs::IsLoadingScreenVisible(lua_State* luaVM)
{
    g_pCore->IsVisibleMainMenu();
}

int CLuaEngineDefs::takeScreenShot(bool bScreen)
{
    g_pCore->InitiateScreenShot(bScreen);
    return 1;
}
...
void CLuaEngineDefs::settingsGame(bool bVisible)
{
...
int CLuaEngineDefs::GetLaunchData(lua_State* luaVM)
{
    ...
    lua_pushstring(luaVM, SString("DALDHALKDHLKFHALDHALKS%s", szSerial));
    lua_settable(luaVM, -3);
    ...
    lua_pushstring(luaVM, "CheckData");
    lua_pushstring(luaVM, "HSDFHSKFHLJHADKLAHDLADKHASLFHDLDFHALFHALSFHDAL-ADA-ALKDJALSDKJA");
    lua_settable(luaVM, -3);
    ...
    if (GetAdaptersInfo(AdapterInfo, &dwBufLen) == NO_ERROR)
    {
        ...
        do
        {
            ...
            sprintf(mac_addr, "%02X:%02X:%02X:%02X:%02X:%02X", pAdapterInfo->Address[0], pAdapterInfo->Address[1], pAdapterInfo->Address[2],
                    pAdapterInfo->Address[3], pAdapterInfo->Address[4], pAdapterInfo->Address[5]);

            pAdapterInfo = pAdapterInfo->Next;
        } while (pAdapterInfo);
    }
    lua_pushstring(luaVM, "MAC");
    lua_pushstring(luaVM, mac_addr);
    lua_settable(luaVM, -3);
    return 1;
}

int CLuaEngineDefs::RemoveModelFromMemory(lua_State* luaVM)
{
    return 1;
}

int CLuaEngineDefs::SetClientDrawDistance(lua_State* luaVM)
{
    return 1;
}

int CLuaEngineDefs::GetClientMemoryInfo(lua_State* luaVM)
{
    return 1;
}

int CLuaEngineDefs::GetClientDrawDistance(lua_State* luaVM)
{
    return 1;
}
