...
void CLocalGUI::SetConsoleVisible(bool bVisible)
{
    if (m_pConsole)
    {
        ...
        else if (IsMainMenuVisible() == false || m_pMainMenu->m_bIsIngame)
            pGUI->SetCursorAlpha(0.0f);
    }
}
...
void CLocalGUI::SetMainMenuVisible(bool bVisible)
{
    if (m_pMainMenu)
    {
        ...
        if (bVisible || m_pMainMenu->m_bIsIngame)
        {
            pGUI->SetCursorAlpha(0.0f);
        }

        if (bVisible == false || !m_pMainMenu->m_bIsIngame)
        {
            pGUI->SetCursorAlpha(1.0f);
        }
    }
