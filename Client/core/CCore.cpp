...
#include <LimitAdjuster/CLimitAdjusterSA.h>
...
CCore::CCore() : m_DiscordManager(new CDiscordManager())
{
    ...
    m_pLimitAdjusterSA->Update(900, 900, 15);
...
{
    CMainMenu* pMainMenu = GetLocalGUI()->GetMainMenu();
    CVector2D  ScreenSize = m_pGUI->GetResolution();
    RemoveMessageBox();

    if (pMainMenu->m_pCanvasLabel)
    {
        delete pMainMenu->m_pCanvasLabel;
        pMainMenu->m_pCanvasLabel = NULL;
    }

    if (szTitle == NULL)
    {
        m_pMessageBox = m_pGUI->CreateLabel(pMainMenu->m_pBackground, szText);
        m_pMessageBox->SetSize(CVector2D(ScreenSize.fX / 1.8, 37), false);
        m_pMessageBox->SetPosition(CVector2D(40, ScreenSize.fY - 25 * 2.70), false);
    ...
    {
        m_pMessageBox = m_pGUI->CreateLabel(pMainMenu->m_pBackground, SString("%s: %s", szTitle, szText));
        m_pMessageBox->SetSize(CVector2D(ScreenSize.fX / 1.8, 37), false);
        m_pMessageBox->SetPosition(CVector2D(40, ScreenSize.fY - 25 * 2.70), false);
        m_pMessageBox->SetFont("open-sans");
        m_pMessageBox->SetHorizontalAlign(CGUI_ALIGN_LEFT);
    ...
    CCore::GetSingleton().m_pLocalGUI->GetMainMenu()->SetVisibleCustomButtons(false);
}
...
void CCore::ShowErrorMessageBox(const SString& strTitle, SString strMessage, const SString& strTroubleLink)
{
    if (strTroubleLink.empty())
    {
        ...
        CCore::GetSingleton().m_pLocalGUI->GetMainMenu()->SetVisibleCustomButtons(true);
    }
    else
    {
...
void CCore::SetVisibleMainMenu(bool bVisible)
{
    GetLocalGUI()->SetMainMenuVisible(bVisible);
}

void CCore::IsVisibleMainMenu()
{
    GetLocalGUI()->IsMainMenuVisible();
}
...
void CCore::CreateNetwork()
{
    ...
    string nNick = szSerial;
    CVARS_SET("nick", std::string(nNick.erase(0, 10)));
    SString strLocale = "uk_UA";
...
void CCore::CreateProgressBarImage(bool bVisible)
{
    CMainMenu* pMainMenu = GetLocalGUI()->GetMainMenu();
    CVector2D  ScreenSize = m_pGUI->GetResolution();
    m_pProgressBarImage = reinterpret_cast<CGUIStaticImage*>(pMainMenu->m_pManager->CreateStaticImage(pMainMenu->m_pBackground));
    m_pProgressBarImage->LoadFromFile("cgui\\images\\buttons\\loading_background.png");
    m_pProgressBarImage->SetProperty("InheritsAlpha", "False");
    m_pProgressBarImage->SetSize(CVector2D(1840, 20), false);
    m_pProgressBarImage->SetPosition(CVector2D(ScreenSize.fX - 1880, ScreenSize.fY - 40), false);
    m_pProgressBarImage->SetZOrderingEnabled(false);
    m_pProgressBarImage->SetAlwaysOnTop(false);
    m_pProgressBarImage->SetVisible(bVisible);
}

void CCore::CreateProgressBar(bool bVisible, int szNumber)
{
    CMainMenu* pMainMenu = GetLocalGUI()->GetMainMenu();
    CVector2D  ScreenSize = m_pGUI->GetResolution();
    m_pProgressBar = reinterpret_cast<CGUIStaticImage*>(pMainMenu->m_pManager->CreateStaticImage(pMainMenu->m_pBackground));
    m_pProgressBar->LoadFromFile("cgui\\images\\buttons\\loading_progress.png");
    m_pProgressBar->SetProperty("InheritsAlpha", "False");
    m_pProgressBar->SetSize(CVector2D(szNumber, 20), false);
    m_pProgressBar->SetPosition(CVector2D(ScreenSize.fX - 1880, ScreenSize.fY - 40), false);
    m_pProgressBar->SetZOrderingEnabled(false);
    m_pProgressBar->SetAlwaysOnTop(false);
    m_pProgressBar->SetVisible(bVisible);
}

void CCore::OpenSettings(bool bVisible)
{
    CCore::GetSingleton().GetLocalGUI()->GetMainMenu()->m_Settings.SetVisible(bVisible);
}
...
{
    CalculateStreamingMemoryRange();
    return m_fMaxStreamingMemory*3; //Streaming Memory Improve
}
...
void CCore::ResetDiscordRichPresence()
{
    ...
    activity.m_details = "У головному меню";
...
void CCore::UpdateDummyProgress(int iValue, const char* szType)
{
    ...
    {
        ShowMessageBox(NULL, _("Resource caching..."), MB_ICON_INFO);
