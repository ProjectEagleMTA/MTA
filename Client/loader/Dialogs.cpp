...
{
    // Remove DLL from GTA directory.
    {
        SString filePath = PathJoin(GetGTAPath(), "VehicleAudioLoader.dll");
        SString filePath1 = PathJoin(GetGTAPath(), "eax.dll");
        SString filePath2 = PathJoin(GetGTAPath(), "ogg.dll");
        SString filePath3 = PathJoin(GetGTAPath(), "version.dll");
        SString filePath4 = PathJoin(GetGTAPath(), "Injector.dll");

        if ((FileExists(filePath)) || (FileExists(filePath1)) || (FileExists(filePath2)) || (FileExists(filePath3)) || (FileExists(filePath4)))
        {
            ((FileDelete(filePath)) || (FileDelete(filePath1)) || (FileDelete(filePath2)) || (FileDelete(filePath3)) || (FileDelete(filePath4)));
        }
    }
#ifndef MTA_DEBUG
